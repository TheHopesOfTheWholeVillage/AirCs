...
#if UNITY_5_6
        get {
            if (imageType==eType.RenderTexture)
                return overlayTextureIds[(currentTextureIndex + bufferCount - 1) % bufferCount]; // Previous buffer
            else
                return overlayTextureIds [currentTextureIndex]; // Current buffer
       } 
#else
